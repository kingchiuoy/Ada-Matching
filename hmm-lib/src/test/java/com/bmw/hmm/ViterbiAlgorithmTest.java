...
public class ViterbiAlgorithmTest {
    ...
    public void testComputeMostLikelySequence() {
        ...
        assertEquals(null, result.get(0).path);
        assertEquals(Descriptor.R2R, result.get(1).path);
        assertEquals(Descriptor.R2S, result.get(2).path);
        assertEquals(Descriptor.S2R, result.get(3).path);
